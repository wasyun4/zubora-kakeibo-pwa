...
function TransactionForm(props: TransactionFormProps) {
    ...
    return (
        <section className="quick-entry">
            ...
            <div className="entry-types" role="group" aria-label="収支の種類">
                {[{id: "expense", label: "支出"}, {id: "income", label: "収入"}].map((item) => (
                    <button key={item.id} type="button" aria-pressed={props.type === item.id}
                        onClick={() => { if (props.type !== item.id) props.onTypeChange(item.id); }}>{item.label}</button>
                ))}
            </div>
            ...
            <div className="entry-fields entry-payment">
                <label>支払方法
